...
class Map_status {
	...
	enum Status_code {
	    ...
	    Illegal_on_slave,		///< operation illegal on a slave host
	    
	    Failure = 15,		///< generic error
	    ...
	    Lookup_no_previous_search	///< no previous search to run again
	};
...
class Map_base {
    public:
	/// \brief all the map file formats, used when writing maps to file.
	enum File_format {
	    Closed_binary = 0,	    ///< proprietary closed format
	    Open_binary,	    ///< public binary format
	    Open_ASCII		    ///< public ASCII format
	};
	...
	/// \param notify is a flag, if true then notify messages will be printed
	/// \param file_format is the format of the map file
	/// \return a status code
	virtual Map_status write(
	    const char		*file_name,
	    const bool		notify = true,
	    const File_format	file_format = Closed_binary) const = 0;
...
class Map_lookup_base {
	...
	/// \brief searches for the closest elements to a given ray,
	/// with the given extension (if less than miHUGE_SCALAR, it
	/// becomes a segment). Works only in 3D.
	/// \param origin is the origin of the ray
	/// \param direction is the direction of the ray
	/// \param extension is the linear extension of the ray
	/// \param max_elements max num of elements to return
	/// \param max_search_dist is the max search distance
	/// from the ray (quadratic)
	/// \return a status code
	virtual Map_status search(
	    const miVector	&origin,
	    const miVector	&direction,
	    const float		extension = miHUGE_SCALAR,
	    const miUint	max_elements = 1,
	    const float		max_search_dist = miHUGE_SCALAR) = 0;

	/// \brief runs the previous search again, returning the
	/// next closest elements.
	/// \return a status code
	virtual Map_status search_next() = 0;
};
