...
/// \brief Assertions and compile-time assertions, see \ref mi_base_assert.
///
//*****************************************************************************
...
/// Compile time assertion that raises a compilation error if
/// the constant expression \p expr evaluates to \c false.
///
/// Example usage: <tt> mi_static_assert(sizeof(char) == 1);</tt>
///
/// This compile-time assertion can be used inside as well as outside
/// of functions.
///
/// If this assertion fails the compiler will complain about applying the
/// \c sizeof operator to an undefined or incomplete type on the line of
/// the assertion failure.
///
/// You may define the macro \c mi_static_assert(expr) yourself
/// to customize its behavior, for example, to disable it.
///
#ifndef mi_static_assert
#ifdef _MSC_VER
// Special case for Visual Studio 7.1, since __LINE__ would not work.
#define mi_static_assert(expr)                                 \
    typedef mi::base::static_assert_test<                      \
        sizeof(mi::base::static_assert_failure<(bool)(expr)>)> \
            MI_BASE_JOIN(static_assert_instance, __COUNTER__)
#else // _MSC_VER
#define mi_static_assert(expr)                                 \
    typedef mi::base::static_assert_test<                      \
        sizeof(mi::base::static_assert_failure<(bool)(expr)>)> \
            MI_BASE_JOIN(static_assert_instance, __LINE__)
#endif // _MSC_VER
...
/// be disabled.
///
/// By default, this macro does nothing. You can (re-)define this macro
...
/// Indicates whether assertions are actually enabled. This symbol gets
/// defined if and only if you (re-)defined \c mi_base_assert and
/// \c mi_base_assert_msg. Note that you can not simply test for
/// \c mi_base_assert or \c mi_base_assert_msg, since these macros get
/// defined in any case (if you do not (re-)define them, they evaluate
/// to a dummy statement that has no effect).
...
/// Expands to a string constant that describes the function in which
/// the macro has been expanded. This can be used as diagnostic
/// in addition to the standard \c __LINE__ and \c __FILE__ values.
/// For compilers that do not support such function name diagnostic
/// the string \c "unknown" will be used.

// If possible, let the asserts support function names in their message.
