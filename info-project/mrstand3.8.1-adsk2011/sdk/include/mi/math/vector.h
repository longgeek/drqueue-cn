...
/// \brief Math vector class template of fixed dimension with arithmetic
///        operators and generic functions, see \ref mi_math_vector.
///
//*****************************************************************************
...
/** Enum used for initializing a \c math::Vector from an iterator.
    \ingroup mi_math
...
// Color and Vector can be converted into each other. To avoid cyclic dependencies
// among the headers, the Color_struct class is already defined here.

//------- POD struct that provides storage for color elements --------

/** \brief Generic storage class template for a RGBA color representation
    storing four elements of type \c mi::Float32.
    \ingroup mi_math_color
    Used as base class for the \c mi::math::Color class.

    Use the \c mi::math::Color class in your programs and this storage class
    only if you need a POD type, for example for parameter passing.

    All elements are usually in the range [0,1], but they may lie outside
    that range and they may also be negative.

    This class provides storage for four elements of type \c
    mi::Float32. These elements can be accessed as data members named
    \c r, \c g, \c b, and \c a. For array-like access of these elements,
    there order in memory is \c rgba.

    This class contains only the data and no member functions.
    See \c mi::math::Color for more.
...
struct Color_struct
{
    ...
    Float32 b;
    /// alpha value, 0.0 is fully transparent and 1.0 is opaque;
    /// can lie outside that range.
    Float32 a;
};
...
/** \defgroup mi_math_vector_struct Internal Storage Class for Math Vector
    ...
    \brief Storage class for %math vectors with support for \c x, \c y, \c z,
           and \c w members for appropriate dimensions.

    Use the \c mi::math::Vector template in your programs and this storage class
    only if you need a POD type, for example for parameter passing.
...
/** \brief Generic storage class template for %math vector representations
    storing \c DIM elements of type \c T.
    Used as base class for the \c mi::math::Vector class template.

    Use the \c mi::math::Vector template in your programs and this storage class
    only if you need a POD type, for example for parameter passing.

    This class template provides array-like storage for \c DIM many
    values of a (arithmetic) type \c T.  It has specializations
    for \c DIM == 1,2,3, and 4. These specializations use data members
    named \c x, \c y, \c z, and \c w according to the dimension. They
    provide users of the \c mi::math::Vector class template with the
    conventional member access to vector elements, such as in:
    \code
...
vec.x = 4.0;
    ...
    \c mi::math::vector_base_ptr(), which is overloaded for the general
...
template <typename T, Size DIM>
inline T * vector_base_ptr(
    Vector_struct<T,DIM>&   vec)   ///< Vector.
{
    return vec.elements;
}
...
/// Returns the base pointer to the vector data.
template <typename T, Size DIM>
inline const T * vector_base_ptr(
    const Vector_struct<T,DIM>&   vec)   ///< Vector.
{
    return vec.elements;
}
...
template <typename T>
inline const T * vector_base_ptr(
    const Vector_struct<T,1>&   vec)   ///< Vector.
{
    return &(vec.x);
}
...
template <typename T>
inline const T * vector_base_ptr(
    const Vector_struct<T,2>&   vec)   ///< Vector.
{
    return &(vec.x);
}
...
template <typename T>
inline const T * vector_base_ptr(
    const Vector_struct<T,3>&   vec)   ///< Vector.
{
    return &(vec.x);
}
...
template <typename T>
inline const T * vector_base_ptr(
    const Vector_struct<T,4>&   vec)   ///< Vector.
{
    return &(vec.x);
}
...
/** Fixed-size %math vector class template with generic operations.
    ...
    This class template provides array-like storage for \c DIM many
    values of an arithmetic type \c T. Several functions and
    arithmetic operators support the work with vectors.

    An instantiation of the vector class template is a model of the
    ...
    Depending on the dimension \c DIM, the \c mi::math::Vector class
    template offers element access through the conventional data members
    named \c x, \c y, \c z, and \c w.  Assuming a vector \c vec of suitable dimension,
    ...
    These data members allow users to access elements, as illustrated in the
    following example:
    \code
mi::math::Vector<mi::Float64,3> vec( 0.0);
vec.x = 4.0;
    ...
    \see
        For the free functions and operators available for vectors and
        vector-like classes see \ref mi_math_vector.
...
public:
    ...
    inline T * begin()
    {
        return mi::math::vector_base_ptr( *this);
    }
    ...
    /// the range over all vector elements.
    inline T * end()
    {
        return begin() + DIM;
    }
    ...
    inline Vector()
    {
        ...
        // In debug mode, default-constructed vectors are initialized with
        // signaling NaNs or, if not applicable, with a maximum value to increase
        // the chances of diagnosing incorrect use of an uninitialized vector.
        //
        // When compiling with Visual C++ 7.1 or earlier, this code is enabled
        // in all variants to work around a very obscure compiler bug that
    ...
    /** \brief Constructor requires the \c mi::FROM_ITERATOR tag as first argument and
        initializes the vector elements with the first \c DIM elements from the
        sequence starting at the iterator \p p.

        \c Iterator must be a model of an input iterator.
        The value type of \c Iterator must be assignment compatible with the vector
        elements type \c T.

        An example:
        \code
std::vector<int> data(10, 42); // ten elements of value 42
mi::math::Vector<mi::Float64, 3> vec( mi::FROM_ITERATOR, data.begin());
        \endcode
    */
    ...
    /** Constructor initializes the vector elements from an \c array of
        dimension \c DIM.

        The value type \c T2 of the \c array must be assignment compatible with
        the vector elements type \c T.
...
mi::math::Vector<mi::Float64, 3> vec(data);
    ...
    */
    ...
    /// other vectors with assignment compatible element value type.
    template <typename T2>
    ...
    /// Dedicated constructor, for dimension 2 only, that initializes
    /// the vector elements from the two elements \c (v1,v2).
    ...
    /// Dedicated constructor, for dimension 3 only, that initializes
    /// the vector elements from the three elements \c (v1,v2,v3).
    ...
    /// Dedicated constructor, for dimension 3 only, that initializes
    /// the vector elements from the three elements \c (v1,v2.x,v2.y).
    /// Precondition: <tt>DIM == 3</tt>.
    ...
    /// Dedicated constructor, for dimension 3 only, that initializes
    /// the vector elements from the three elements \c (v1.x,v1.y,v2).
    /// Precondition: <tt>DIM == 3</tt>.
    ...
    /// Dedicated constructor, for dimension 4 only, that initializes
    /// the vector elements from the four elements \c (v1,v2,v3,v4).
    /// Precondition: <tt>DIM == 4</tt>.
    ...
    /// Dedicated constructor, for dimension 4 only, that initializes
    /// the vector elements from the four elements \c (v1,v2,v3.x,v3.y).
    /// Precondition: <tt>DIM == 4</tt>.
    ...
    /// Dedicated constructor, for dimension 4 only, that initializes
    /// the vector elements from the four elements \c (v1,v2.x,v2.y,v3).
    /// Precondition: <tt>DIM == 4</tt>.
    ...
    /// Dedicated constructor, for dimension 4 only, that initializes
    /// the vector elements from the four elements \c (v1.x,v1.y,v2,v3).
    /// Precondition: <tt>DIM == 4</tt>.
    ...
    /// Dedicated constructor, for dimension 4 only, that initializes
    /// the vector elements from the four elements \c (v1.x,v1.y,v2.x,v2.y).
    /// Precondition: <tt>DIM == 4</tt>.
    ...
    /// Dedicated constructor, for dimension 4 only, that initializes
    /// the vector elements from the four elements \c (v1,v2.x,v2.y,v2.z).
    /// Precondition: <tt>DIM == 4</tt>.
    ...
    /// Dedicated constructor, for dimension 4 only, that initializes
    /// the vector elements from the four elements \c (v1.x,v1.y,v1.z,v2).
    /// Precondition: <tt>DIM == 4</tt>.
    ...
    /// Dedicated constructor, for dimension 4 only, that initializes
    /// the vector elements from a color interpreted as a vector (r,g,b,a).
    /// Precondition: <tt>DIM == 4</tt>.
    ...
    /// Assignment, for dimension 4 only, that assigns
    /// color interpreted as a vector (r,g,b,a) to this vector.
    /// Precondition: <tt>DIM == 4</tt>.
    ...
    /// Accesses the \c i-th vector element, <tt>0 <= i < size()</tt>.
    ...
    /// Returns the \c i-th vector element, <tt>0 <= i < size()</tt>.
    ...
    /// Sets the \c i-th vector element to \c value, <tt>0 <= i < size()</tt>.
    ...
    /// Normalizes this vector to unit length. Returns \c false if normalization
    /// fails because the vector norm is zero, and \c true otherwise. This vector
    /// remains unchanged if normalization failed.
    ///
    ...
    inline bool operator== (
        Vector_proxy_<T,DIM> rhs) const  // right hand-side vector
    {
    ...
    /// Returns \c true if \c lhs is \ref LexOrder "lexicographically greater than"
    /// \c rhs.
    inline bool operator> (
        Vector_proxy_<T,DIM> rhs) const // right hand-side vector
    {
    ...
    /// Returns \c true if \c lhs is
    /// \ref LexOrder "lexicographically greater than or equal" to \c rhs.
    inline bool operator>= (
        Vector_proxy_<T,DIM> rhs) const // right hand-side vector
    {
        return ! lexicographically_less( *this, rhs.vec);
    ...
    // Sets the fields of the vector to the fields of the \p other vector.
    // Used for the C++ Plugin API binding only. Deprecated.
    inline void set_fields( const Vector<Float32,2> other)
    {
        mi_static_assert(DIM >= 2 && DIM <= 4);
        for (Size i(0u); i < DIM; ++i)
            (*this)[i] = (i<2) ? T(other[i]) : T(0);
    }

    // Sets the fields of the vector to the fields of the \p other vector.
    // Used for the C++ Plugin API binding only. Deprecated.
    inline void set_fields( const Vector<Float64,2> other)
    {
        mi_static_assert(DIM >= 2 && DIM <= 4);
        for (Size i(0u); i < DIM; ++i)
            (*this)[i] = (i<2) ? T(other[i]) : T(0);
    ...
    // Sets the fields of the vector to the fields of the \p other vector.
    // Used for the C++ Plugin API binding only. Deprecated.
    inline void set_fields( const Vector<Float32,3> other)
    {
        mi_static_assert(DIM >= 2 && DIM <= 4);
        for (Size i(0u); i < DIM; ++i)
            (*this)[i] = (i<3) ? T(other[i]) : T(0);
    }

    // Sets the fields of the vector to the fields of the \p other vector.
    // Used for the C++ Plugin API binding only. Deprecated.
    inline void set_fields( const Vector<Float64,3> other)
    {
        mi_static_assert(DIM >= 2 && DIM <= 4);
        for (Size i(0u); i < DIM; ++i)
            (*this)[i] = (i<3) ? T(other[i]) : T(0);
    ...
    // Sets the fields of the vector to the fields of the \p other vector.
    // Used for the C++ Plugin API binding only. Deprecated.
    inline void set_fields( const Vector<Float32,4> other)
    {
        mi_static_assert(DIM >= 2 && DIM <= 4);
        for (Size i(0u); i < DIM; ++i)
            (*this)[i] = (i<4) ? T(other[i]) : T(0);
    }

    // Sets the fields of the vector to the fields of the \p other vector.
    // Used for the C++ Plugin API binding only. Deprecated.
    inline void set_fields( const Vector<Float64,4> other)
    {
        mi_static_assert(DIM >= 2 && DIM <= 4);
        for (Size i(0u); i < DIM; ++i)
            (*this)[i] = (i<4) ? T(other[i]) : T(0);
    ...
    // Sets the fields of the vector to the \p scalar value.
    // Used for the C++ Plugin API binding only. Deprecated.
    inline void set_fields( T scalar)
    {
        for (Size i(0u); i < DIM; ++i)
            (*this)[i] = scalar;
    }

    // Sets the fields of the vector, for dimension 4 only, to the color
    // interpreted as a vector (r,g,b,a).
    // Precondition: <tt>DIM == 4</tt>.
    // Used for the C++ Plugin API binding only. Deprecated.
    inline void set_fields( const Color_struct color)
    {
        mi_static_assert(DIM >= 2 && DIM <= 4);
        begin()[0] = color.r;
        begin()[1] = color.g;
    ...
    // Sets the fields of the vector, for dimension 2 only, to the vector
    // of the four elements \c (v1,v2).
    // Precondition: <tt>DIM == 2</tt>.
    // Used for the C++ Plugin API binding only. Deprecated.
    inline void set_fields(T v1, T v2)
    {
        mi_static_assert(DIM == 2);
        begin()[0] = v1;
        begin()[1] = v2;
    ...
    // Sets the fields of the vector, for dimension 3 only, to the vector
    // of the four elements \c (v1,v2,v3).
    // Precondition: <tt>DIM == 3</tt>.
    // Used for the C++ Plugin API binding only. Deprecated.
    inline void set_fields(T v1, T v2, T v3)
    {
        mi_static_assert(DIM == 3);
        begin()[0] = v1;
        begin()[1] = v2;
    ...
    // Sets the fields of the vector, for dimension 4 only, to the vector
    // of the four elements \c (v1,v2,v3,v4).
    // Precondition: <tt>DIM == 4</tt>.
    // Used for the C++ Plugin API binding only. Deprecated.
    inline void set_fields(T v1, T v2, T v3, T v4)
    {
        mi_static_assert(DIM == 4);
        begin()[0] = v1;
        begin()[1] = v2;
    ...
    // Divide all components by a factor. Division by zero are ignored
    // Used for the C++ Plugin API binding only. Deprecated.
    ...
    // This method exists only for Vector3 and Dvector3.
    // Used for the C++ Plugin API binding only. Deprecated.
    ...
    // Computes and return the dot product with another vector.
    // Used for the C++ Plugin API binding only. Deprecated.
    ...
    // Returns this vector clamped component-wise to the range [lower,upper],
    // that means no component of the returned vector is less than lower or
    // greater than upper.
    // Used for the C++ Plugin API binding only. Deprecated.
    inline Vector clamp (Vector lower, Vector upper) const;

    // Returns the component-wise maximum of this vector and the lower bound.
    // Used for the C++ Plugin API binding only. Deprecated.
    inline Vector max MI_PREVENT_MACRO_EXPAND (Vector lower) const;

    // Returns the component-wise minimum of this vector and the upper bound.
    // Used for the C++ Plugin API binding only. Deprecated.
...
template <typename T, Size DIM>
inline Vector<T,DIM> operator- (
    const Vector<T,DIM> & v)    // vector to negate
{
    Vector<T,DIM> tmp;
    for (Size i(0u); i < DIM; ++i)
        tmp[i] = -v[i];
...
/// Multiplies the vector \p v elementwise with the scalar \p s and returns
/// the modified vector \p v.
template <typename T, typename TT, Size DIM>
inline Vector<T,DIM> & operator*= (
    Vector<T,DIM> &     v,      // left hand-side vector
    TT                  s)      // right hand-side scalar
{
    for (Size i(0u); i < DIM; ++i)
        v[i] = T(v[i] * s);
    return v;
...
/// Divides the vector \p v elementwise by the scalar \p s and returns the
/// modified vector \p v.
template <typename T, typename TT, Size DIM>
inline Vector<T,DIM> & operator/= (
    Vector<T,DIM> &     v,      // left hand-side vector
...
template <typename T, typename TT, Size DIM>
inline Vector<T,DIM> operator* (
    const Vector<T,DIM> & v,    // left hand-side vector
    TT                    s)    // right hand-side scalar
{
    Vector<T,DIM> tmp(v);
    return tmp *= s;
...
template <typename T, typename TT, Size DIM>
inline Vector<T,DIM> operator* (
    TT                    s,    // left hand-side scalar
    const Vector<T,DIM> & v)    // right hand-side vector
{
    Vector<T,DIM> tmp(v);
    return tmp *= s;
}
...
inline Vector<bool,DIM> operator&& (
    const Vector<bool,DIM> & lhs,    // left  hand-side vector
    const Vector<bool,DIM> & rhs)    // right hand-side vector
{
...
template <Size DIM>
inline Vector<bool,DIM> operator! (
    const Vector<bool,DIM> & vec)    // vector
{
    Vector<bool,DIM> result;
...
/// Returns \c true if \c all of all elements of \c v returns true.
template <typename T, Size DIM>
...
/// Returns \c true if \c any of any element of \c v returns true.
template <typename T, Size DIM>
...
/// The signs of the elements of \p v and \p w are used to determine
/// the quadrant of the results.
template <typename T, Size DIM>
...
/// Returns a vector with the elementwise smallest integral value that is not
/// less than the element in vector \p v.
template <typename T, Size DIM>
...
/// Returns a vector with the elementwise largest integral value that is not
/// greater than the element in vector \p v.
template <typename T, Size DIM>
...
inline bool is_approx_equal(
    const Vector<T,DIM> & left,    // left hand-side
    const Vector<T,DIM> & right,   // right hand-side
    T                             e) // true if closer than this epsilon
{
    for (Size i = 0u; i < DIM; ++i)
...
/// Returns the elementwise linear interpolation between \p v1 and \c v2,
...
/// Returns the elementwise fractional part of \p v and stores the elementwise
/// integral part of \p v in \p i; both parts have elementwise the same sign as \p v.
template <typename T, Size DIM>
...
/// Returns 0 if \p v is less than \p a and 1 if \p v is greater than \p b
/// in an elementwise fashion.
/// A smooth curve is applied in-between so that the return values vary
/// continuously from 0 to 1 as elements in \p v vary from \p a to \p b.
template <typename T, Size DIM>
...
template <typename T>
inline T cross(
    const Vector<T,2> & lhs,    // left hand-side vector
    const Vector<T,2> & rhs)    // right hand-side vector
{
    return lhs.x * rhs.y - lhs.y * rhs.x;
}

/// Returns the three-dimensional cross product result for the two vectors
/// \p lhs and \p rhs.
template <typename T>
inline
Vector<T,3> cross(
    const Vector<T,3> & lhs,    // left hand-side vector
    const Vector<T,3> & rhs)    // right hand-side vector
{
    return Vector<T,3>
...
/// Given a unit length vector \p n, computes two vectors \p u and \p v such that
/// (\p u, \p n, \p v) forms an orthonormal basis (\p u and \p v are unit length). This
/// function is not continuous with respect to \p n: in some cases, a small
/// perturbation on \p n will flip the basis.
template <typename T>
...
/// Given a unit length vector n, and two non-colinear and non-zero vectors u
/// and v, this function computes two vectors t and b such that (t, n, b)
/// forms an orthonormal basis (t and b are unit length), t lies in the plane
/// formed by n and u, and b has the same orientation as v (i.e. dot(b, v) >= 0).
template <typename T>
inline void make_basis(
    const Vector<T,3> & n,     ///< input, normal vector
...
/// Converts the vector \p v of type \c Vector<T1, DIM1> to
/// a vector of type \c Vector<T2, DIM2>.
/// If \p DIM1 < \p DIM2, the remaining values are filled
/// with \p fill. If \p DIM1 > \p DIM2, the values that
/// don't fit into the result vector are discarded.
/// The conversion from \p T1 to \p T2 must be possible.
