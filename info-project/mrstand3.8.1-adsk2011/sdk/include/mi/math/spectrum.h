...
/// \brief Spectrum class with \c mi::Float32 elements and
///        operations, see \ref mi_math_spectrum.
///
//*****************************************************************************
...
/** \defgroup mi_math_spectrum Spectrum Class
    \ingroup mi_math
    Spectrum class with \c mi::Float32 elements and operations.
...
/** Spectrum with \c mi::Float32 elements and operations.

    \note
    This class will most likely change in the future, since currently
    it uses only three components. Therefore it behaves almost identical
    to the Color class, apart from the missing alpha component.
    Future implementations will probably contain a lot more bands
    or might have a completely different internal structure.

    The spectrum class is a model of the STL container concept. It provides
    random access to its elements and corresponding random access iterators.

    \see
        For the free functions and operators available for spectra see
        \ref mi_math_spectrum.
...
public:
    ...
    /// the range over all spectrum elements.
    ...
    inline Spectrum()
    {
        ...
        // In debug mode, default-constructed spectra are initialized with
        // signaling NaNs or, if not applicable, with a maximum value to increase
        // the chances of diagnosing incorrect use of an uninitialized spectrum.
        //
    ...
    /// Conversion from Vector3 to Spectrum.
    ...
    /// Conversion from Vector4 to Spectrum.
    ...
    /// Conversion from Color to Spectrum.
    ...
    /// Conversion from Spectrum to Vector3.
    ...
    /// Conversion from Spectrum to Vector4.
...
inline Spectrum & operator+= (
    Spectrum &       lhs,  // left hand-side spectrum
    const Spectrum & rhs)  // right hand-side spectrum
{
...
inline Spectrum operator+ (
    const Spectrum & lhs,  // left hand-side spectrum
    const Spectrum & rhs)  // right hand-side spectrum
{
    mi_math_assert_msg(lhs.size() == 3, "precondition");
    mi_math_assert_msg(rhs.size() == 3, "precondition");
...
/// Negates the spectrum \p v elementwise and returns the new result.
inline Spectrum operator- (
    const Spectrum & c)    // spectrum to negate
{
...
/// Multiplies the spectrum \p v elementwise with the scalar \p s and returns
/// the modified spectrum \p v.
...
/// Divides the spectrum \p v elementwise by the scalar \p s and returns the
/// modified spectrum \p v.
inline Spectrum & operator/= ( Spectrum & c, Float32 s)
{
    mi_math_assert_msg(c.size() == 3, "precondition");
    Float32 f = 1.0f / s;
...
/// Divides the spectrum \p v elementwise by the scalar \p s and returns the new result.
inline Spectrum operator/ ( const Spectrum & c, Float32 s)
{
    mi_math_assert_msg(c.size() == 3, "precondition");
    Float32 f = 1.0f / s;
...
/// Returns a spectrum with the elementwise smallest integral value that is not
/// less than the element in spectrum \p c.
...
/// Returns elementwise max for each element in spectrum \p lhs that is less
/// than the corresponding element in spectrum \p rhs.
...
/// Returns elementwise min for each element in spectrum \p lhs that is less
/// than the corresponding element in spectrum \p rhs.
...
/// Returns a spectrum with the elementwise largest integral value that is not
/// greater than the element in spectrum \p c.
...
/// Returns a gamma corrected spectrum. Gamma factors are used to correct for
...
/// the spectrum components to the gamma exponent. Gamma factors greater than 1
...
inline bool is_approx_equal(
    const Spectrum& lhs,   // left hand-side
    const Spectrum& rhs,   // right hand-side
...
/// Return whether all components of the spectrum are finite.
...
/// Return whether any component of the spectrum is infinite.
...
/// Return whether any component of the spectrum is "not a number".
