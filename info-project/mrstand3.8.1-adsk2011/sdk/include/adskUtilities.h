...
{
    if (pFrameBufferWriteOperation == FB_WRITE_NOOP ||
        pFrameBufferInfo.flags & FB_FLAG_CUSTOM_PASS_EVAL) return miTRUE;
...
inline void operateOnFrameBuffersOfType(const miState *pState,
                                        ...
                                        const bool &pUseIrradiance,
                                        const miColor* const& pIrradianceNoShadow = 0 ){
    if (!pFrameBufferInfo || !pPassTypeInfo || pPassTypeInfo[pTypeID].count == 0
        || pFrameBufferWriteOperation == FB_WRITE_NOOP ||
        pFrameBufferInfo[0].flags & FB_FLAG_CUSTOM_PASS_EVAL) return;
...
inline void operateOnNormalMaterialFrameBuffersOfType(const miState *pState,
                                                      ...
                                                      const miScalar &pFrameBufferWriteFactor)
{
    if (!pFrameBufferInfo || !pPassTypeInfo ||
        pFrameBufferWriteOperation == FB_WRITE_NOOP ||
        pFrameBufferInfo[0].flags & FB_FLAG_CUSTOM_PASS_EVAL) return;

    miInteger lCount;
    miUint lPass;
    for (lCount = 0, lPass = pPassTypeInfo[NORMAL].start;
         lCount < pPassTypeInfo[NORMAL].count;
         ++lCount, ++lPass){

        // get the parameter associated with this normal material pass

        miInteger lNormalMode = pFrameBufferInfo[lPass].integerUserParameter1;
        miVector lResult;
        switch (lNormalMode){
        case 0:                 // world
            mi_normal_to_world(pState, &lResult, &pValue);
            break;
        case 1:                 // camera
            mi_normal_to_camera(pState, &lResult, &pValue);
            break;
        case 2:                 // object
            mi_normal_to_object(pState, &lResult, &pValue);
            break;
...
/*!
  \brief Check if the pass is the given type

  \param[in] pPassTypeInfo PassTypeInfo.

  \param[in] pPassID the pass index.

  \param[in] pTypeID pass type.

  \return x true if the pass, pPassID is the given type pTypeID. false otherwise
*/
inline bool checkPassType(PassTypeInfo* const pPassTypeInfo, unsigned int pPassID, unsigned int pTypeID)
{
	return ((pPassID >= (unsigned int)pPassTypeInfo[pTypeID].start) && 
		(pPassID < (unsigned int)(pPassTypeInfo[pTypeID].start + pPassTypeInfo[pTypeID].count)));
}
...
/*!
  ...
  \param[in] pVolumeShadows Must be set to true when evaluating volume shadows
  \param[in] pState the shading state
