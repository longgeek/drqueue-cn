...
#define PASS_SHADER_BASE_PARAMETERS             \
    ...
    miBoolean   holdout;                        \
    miBoolean   useShadingEngineThreshold;      \
    miScalar    transparencyThreshold;
                         ...
                         PassShaderParameters *pParameters)
{
    ...
    if (lRayFilter())
    {
        //Note: Threshold check requires the pass to be a post material pass
        //to work correctly
        bool meetsThreshold = true;
        FrameBufferState* fbState = getFrameBufferState(state);
        if (fbState)
        {
            if (*mi_eval_boolean(&pParameters->useShadingEngineThreshold))
            {
                meetsThreshold = !fbState->pretendNoHit;
            }
            else
            {
                meetsThreshold = MAYA_LUMINANCE(fbState->transparency) <= 
                    (1.0f - *mi_eval_scalar(&pParameters->transparencyThreshold) + MAYA_EPS);
            }
        }
        ...
        if (meetsThreshold)
        {
            ...
            if (!contributes && *mi_eval_boolean(&pParameters->holdout) )
            {
                // In holdout mode, we make sure to overwrite the contibution from
                // deeper trace recursions.
                // Note: holdout will only work correctly for post-material pass shaders.
                mi_fb_put(state, mFrameBufferID, const_cast<miColor*>(&BLACK));
            }
        } //meets threshold
    } // ray filter
  ...
  public:
    ...
    */
    PassShaderEyeRayFilter(miState* pState, PassShaderParameters* pParameters) {
        ...
        mFilterResult = (
            inTileRenderingStage &&
            (pState->type == miRAY_EYE || 
                (pState->type == miRAY_TRANSPARENT && 
                pState->reflection_level == 0 &&
                pState->refraction_level == 0)
            ));
    }
    /*!
