...
enum FrameBufferFlagBits {
    ...
    FB_FLAG_SHADOW_PASS =             1 << 13, //!< The render pass is illuminated by shadow light differentials
    FB_FLAG_DISABLE =                 1 << 14, //!< Turn off frame buffer contribution (global), useful for invoking upstream shaders
    FB_FLAG_NO_COMPOSITING =          1 << 15, //!< Indicates a render pass type that does not support any form of compositing
    FB_FLAG_CUSTOM_PASS_EVAL =        1 << 16, //!< Indicates that the shading engine is currently evaluating custom render pass nodes in forced evaluation mode
    FB_FLAG_CUSTOM_PASS_EVAL_GUARD =  1 << 17  //!< Recursion guard for custom render passes in forced evaluation mode
};
...
ADSK_SHADERSDK_EXPORT const char* const FRAME_BUFFER_STATE_KEY;

/*!
  \brief Data structure that stores state information used
  for frame buffer management.
*/
struct FrameBufferState
{
    miBoolean allocateTempData;  //!< Indicate whether temp data should be allocated on the heap
    miBoolean pretendNoHit;      //!< Result of cutaway opacity calculation
    miColor   transparency;      //!< Transparency of the current intersection, set by shading engine
};
...
/*!
  \brief Retrieve state information used for frame buffer management

  \param[in] pState the shading state

  \return reference to the frame buffer state structure
*/
ADSK_SHADERSDK_EXPORT
...
/*!
  \brief Creating an object of this class will disable frame buffer 
  contributions in indirect shader calls in the current thread for the 
  lifetime of the object

  This class controls the the FB_FLAG_DISABLE flag in a way that is
  controlled by variable scope, which makes it exception-safe and
  return/break safe.  It also takes care of saving and restoring the prior
  state of the flag.

  This class is convenient to use when evaluating shader inputs and
  it is not desirable for the input evaluation to trigger frame buffer
  contributions via indirect upstream shader calls.
*/
class FrameBufferDisable
{
public:
    inline FrameBufferDisable(const unsigned int pNumberOfFrameBuffers, FrameBufferInfo* pFrameBufferInfo)
    {
        start(pNumberOfFrameBuffers, pFrameBufferInfo);
    }
    
    inline ~FrameBufferDisable()
    {
        done();
    }
    ...
    inline void done()
    {
        if (mFlagsToTurnBackOff)
        {
            (*mFlagsToTurnBackOff) &= ~FB_FLAG_DISABLE;
            mFlagsToTurnBackOff = 0; // to only do this once
        }
    }

    inline void restart(const unsigned int pNumberOfFrameBuffers, FrameBufferInfo* pFrameBufferInfo)
    {
        done();
        start(pNumberOfFrameBuffers, pFrameBufferInfo);
    }
...
private:
    inline void start(const unsigned int pNumberOfFrameBuffers, FrameBufferInfo* pFrameBufferInfo)
    {
        if (pNumberOfFrameBuffers && !(pFrameBufferInfo[0].flags & FB_FLAG_DISABLE))
        {
            mFlagsToTurnBackOff = &pFrameBufferInfo[0].flags;
            (*mFlagsToTurnBackOff) |= FB_FLAG_DISABLE;
        }
        else
        {
            mFlagsToTurnBackOff = NULL;
        }
    }

    FrameBufferFlags* mFlagsToTurnBackOff;
};
