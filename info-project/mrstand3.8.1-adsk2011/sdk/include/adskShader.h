  ...
  public:
    ...
    */
                                                ...
                                                void *pLightBlindData)
    {
        ...
        // full qualification
        miState * const &state = LambertianShaderHelper<ShaderParameters>::state;
        float factor = arealight_lambertian_reflect(&state->point, &pBumpNormal, pLightBlindData);
            ...
            while (mi_sample_light(&lightColor, &lightDirection, &dotNL, state, *lights, &samples))
            {
                ...
                if (inLightData->lightDiffuse)
                {
                    ...
                    if (dotNL > 0.f)
                    {
                        ...
                        MaterialBase::writeToFrameBuffers(state, frameBufferInfo, passTypeInfo, 
                                                          opaqueColor(irradianceFactor), DIRECT_IRRADIANCE_NO_SHADOW, true);
                        MaterialBase::writeToFrameBuffers(state, frameBufferInfo, passTypeInfo, 
                                                          opaqueColor(irradianceFactor), RAW_SHADOW, true);
                        MaterialBase::writeToFrameBuffers(state, frameBufferInfo, passTypeInfo, 
                                                          dotNL, RAW_SHADOW_MONO, true);
                    }
                    ...
                    if (translucence)
                    {
                        ...
                        if (!blackHole)
                        {
                            ...
                            MaterialBase::writeToFrameBuffers(state, frameBufferInfo, passTypeInfo, 
                                                              opaqueColor(unlitTranslucence), TRANSLUCENCE_NO_SHADOW, true);
                        }
                    ...
                    if (!blackHole)
                    {
                        ...
                        MaterialBase::writeToFrameBuffers(state, frameBufferInfo, passTypeInfo, 
                                                          opaqueColor(unlitDiffuse + unlitTranslucence), DIFFUSE_NO_SHADOW, true);
                    }
                }
                ...
                if (inLightData->lightSpecular)
                {
                    ...
                    if (*lightBlindData)
                    {
                        unlitSpecular = factor * unlitSpecular;
                    }
                    ...
                    if ( !blackHole )
                    {
                        ...
                        MaterialBase::writeToFrameBuffers(state, frameBufferInfo, passTypeInfo, 
                                                          opaqueColor(unlitSpecular), SPECULAR_NO_SHADOW, true);
                    }
                }
                ...
                {
                    MaterialBase::writeToFrameBuffers(state, frameBufferInfo, passTypeInfo, 
                                                      opaqueColor(unlitBeauty), SHADOW, true);
                    MaterialBase::writeToFrameBuffers(state, frameBufferInfo, passTypeInfo, 
                                                      MAYA_LUMINANCE(unlitBeauty), SHADOW_MONO, true);
                    MaterialBase::writeToFrameBuffers(state, frameBufferInfo, passTypeInfo, 
                                                      opaqueColor(unlitBeauty), BEAUTY, true );
                    MaterialBase::writeToFrameBuffers(state, frameBufferInfo, passTypeInfo, 
                                                      opaqueColor(unlitBeauty), BEAUTY_NO_SHADOW, true );
                }
