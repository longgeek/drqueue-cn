...
typedef enum miModule {
	...
	miM_SAMP,		/* sampling routines */
        ...
        miM_RAST,		/* rasterizer */
	miM_DISK,		/* disk routines */
	miM_MRMSL,		/* mental ray metasl proxy */
	miM_CL12,		/* metasl level 1+2 graph flattener */
	miM_SHADERDB,		/* metasl shader database */
	miM_MSLC,		/* metasl compiler proxy */
	miM_CB12,		/* metasl level 1+2 compiler backend */
	miM_BMSL,		/* metasl compiler bin file generator */
	miM_COMP,		/* metasl compiler kernel */
	miM_CF12,		/* metasl level 1+2 compiler frontend */
	miM_GSWRC,		/* metasl compiler C++ code generator */
	miM_SYNT,		/* metasl compiler syntax tree */
	miM_CT12,		/* metasl level 1+2 compiler syn.tree preproc. */
	miM_HALF,		/* 16-bit float type */
	miM_RCMAP,		/* RC map */
...
#define RAY37                   /* for shaders that share sources with 3.7 */
#define RAY38                   /* for shaders that share sources with 3.8 */
...
miScalar	mi_spline		(miScalar, int, miScalar *);
...
miScalar	mi_unoise_3d		(miVector *);
...
miScalar	mi_unoise_2d_grad	(miScalar, miScalar,
					 miScalar *, miScalar *);
miScalar	mi_noise_3d_grad	(miVector *, miVector *);
miScalar	mi_unoise_3d_grad	(miVector *, miVector *);
...
void		mi_nfatal		(const int, const char *, ...);
void		mi_nerror		(const int, const char *, ...);
void		mi_nwarning		(const int, const char *, ...);
...
typedef struct miImg_file {
	...
	miCBoolean	spare2;
	int		spare[10];	/* makes sizeof(miImg_file) 256 bytes */
} miImg_file;
...
#define miRC_BUFFER_PREMULT     0x8 /* rasterizer: use premultiplication 
				       mode for compositing. */

typedef struct miFb_info {
	enum miImg_type	image_type;		/* image types */
						/* miIMG_TYPE_RGBA, ...ANY...*/
	miUint4		write_image;		/* OR'ed miRC_BUFFER_* flags */
	miCBoolean	interp_image;		/* interpolate image? */
	miCBoolean	has_cprof;		/* colorprofile present? needed in mi_fb_put/get*/
	miCBoolean	spare[2];
} miFb_info;
...
class Allocatable
{
  public:
	...
	int	    line = __LINE__)
	{return mi_mem_int_allocate((char *)file, line, (unsigned int)sz);}
    ...
    void operator delete (	      /* Some compilers fall apart when they */
	void		*a,	      /* see a placement delete.             */
        void*)
	{}
